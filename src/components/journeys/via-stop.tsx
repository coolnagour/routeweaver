...
export default function ViaStop({ 
  ...
  // Destination is always editable. Other stops are locked if `isLocked` is true.
  const isThisStopLocked = isLocked && !isDestination;
    ...
    <div className="p-4 border rounded-lg space-y-3 bg-muted/20 relative">
        <div className="flex justify-between items-center">
            ...
            {!isDestination && removeStop && (
                <Button type="button" variant="ghost" size="icon" className="h-9 w-9 text-destructive" onClick={() => removeStop(index)} disabled={isThisStopLocked}>
        ...
        <div className="flex items-start gap-2">
          ...
          {isPickup ? (
                 ...
                 <FormItem>
                     ...
                     <FormControl>
                         <div className="relative flex items-center">
                             ...
                             <Input placeholder="e.g. John Smith" {...field} value={field.value ?? ''} className="pl-10 pr-10 bg-background" disabled={isThisStopLocked} />
                             <Button type="button" variant="ghost" size="icon" className="absolute right-1 h-8 w-8 text-primary" onClick={() => onGenerateField('name', `stops.${index}.name`, index)} disabled={generatingFields[`stops.${index}.name-name`] || isThisStopLocked}>
                 ...
                 <FormItem>
                     ...
                     <FormControl>
                         <div className="relative flex items-center">
                             ...
                             <Input placeholder="e.g. +15551234567" {...field} value={field.value ?? ''} className="pl-10 pr-10 bg-background" disabled={isThisStopLocked} />
                             <Button type="button" variant="ghost" size="icon" className="absolute right-1 h-8 w-8 text-primary" onClick={() => onGenerateField('phone', `stops.${index}.phone`, index)} disabled={generatingFields[`stops.${index}.phone-phone`] || isThisStopLocked}>
             ...
             <FormField
                 ...
                 <FormItem>
                     ...
                     <Select onValueChange={field.onChange} defaultValue={field.value} disabled={isThisStopLocked}>
                         <FormControl>
          ...
          <Collapsible>
             <CollapsibleTrigger asChild>
                 <Button variant="link" size="sm" className="p-0 h-auto" disabled={isThisStopLocked}>
                        ...
                        <FormItem>
                            ...
                            <Popover>
                                ...
                                <Calendar
                                    ...
                                    disabled={(date) => date < new Date(new Date().setHours(0,0,0,0)) || isThisStopLocked}
                                    initialFocus
                                />
                                </PopoverContent>
                            </Popover>
                         ...
                         <FormItem>
                             ...
                             <FormControl>
                                 <div className="relative flex items-center">
                                     ...
                                     <Input placeholder="e.g., Gate code #1234" {...field} value={field.value ?? ''} className="pl-10 pr-10 bg-background" disabled={isThisStopLocked}/>
                                     <Button type="button" variant="ghost" size="icon" className="absolute right-1 h-8 w-8 text-primary" onClick={() => onGenerateField('instructions', `stops.${index}.instructions`, index, stopType)} disabled={generatingFields[`stops.${index}.instructions-instructions`] || isThisStopLocked}>
